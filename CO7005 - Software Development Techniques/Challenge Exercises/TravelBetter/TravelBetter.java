...
int[] out = new int[travellers.length];
int[] in = new int[travellers.length];
for (int i = 0; i < travellers.length; i++) {
	people.add(travellers[i][1]);
	for (int j = 0; j < trainsOut.length - 1; j++) {
		if (trainsOut[j][0] == people.get(i) && trainsOut[j + 1][0] == people.get(i)) {
			// out.add(Integer.parseInt(trainsOut[j][5]) > Integer.parseInt(trainsOut[j + 1][5]) ? trainsOut[j][5] : trainsOut[j + 1][5]);
			out[i] = Integer.parseInt(trainsOut[j][5]) > Integer.parseInt(trainsOut[j + 1][5]) ? j : j + 1;
		}
		if (trainsOut[j][1] == people.get(i) && trainsOut[j + 1][1] == people.get(i)) {
			// in.add(Integer.parseInt(trainsRtn[j][5]) > Integer.parseInt(trainsRtn[j + 1][5]) ? trainsRtn[j][5] : trainsRtn[j + 1][5]);
			in[i] = Integer.parseInt(trainsRtn[j][5]) > Integer.parseInt(trainsRtn[j + 1][5]) ? j : j + 1;
		} 
	}
}
...
for (int i = 0; i < people.size() - 1; i++) {
	...
	System.out.println("Outbound Departure: " + trainsOut[out[i]][2]);
	System.out.println("Outbound Arrival: " + trainsOut[out[i]][3]);
	System.out.println("Inbound Departure: " + trainsRtn[in[i]][2]);
	System.out.println("Inbound Arrival: " + trainsRtn[in[i]][3]);
	int temp = Integer.parseInt(trainsOut[i][5]) + Integer.parseInt(trainsRtn[i][5]);
